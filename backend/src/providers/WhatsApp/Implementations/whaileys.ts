...
const getChats = async (sessionId: number): Promise<{ id: string; name: string; isGroup: boolean; unreadCount: number; timestamp: number }[]> => {
  try {
    const wbot = getWbot(sessionId);
    // groupFetchAllParticipating retrieves all groups the user is currently in
    ...
    return Object.values(groups).map((group: any) => ({
      ...
      name: group.subject,
      isGroup: true,
      unreadCount: 0, // Baileys doesn't easily expose unread count per group without a full store
      timestamp: group.creation || 0
    }));
  } catch (err) {
    logger.error(`Error fetching chats in Baileys: ${err}`);
    return [];
  }
};
